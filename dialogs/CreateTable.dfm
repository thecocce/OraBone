inherited CreateTableDialog: TCreateTableDialog
  Caption = 'Create Table'
  ClientHeight = 477
  ClientWidth = 688
  ExplicitWidth = 704
  ExplicitHeight = 515
  PixelsPerInch = 96
  TextHeight = 13
  object TableNameLabel: TLabel [0]
    Left = 8
    Top = 12
    Width = 56
    Height = 13
    Caption = 'Table Name'
  end
  object CommentLabel: TLabel [1]
    Left = 8
    Top = 36
    Width = 45
    Height = 13
    Caption = 'Comment'
  end
  inherited PageControl: TBCPageControl
    Top = 62
    Width = 681
    Height = 360
    ActivePage = ColumnCommentsTabSheet
    ActivePageCaption = 'Column Comments'
    ExplicitTop = 62
    ExplicitWidth = 681
    ExplicitHeight = 360
    object ColumnsTabSheet: TTabSheet [0]
      Caption = 'Columns'
      ImageIndex = 1
      object ColumnsPanel: TPanel
        Left = 0
        Top = 0
        Width = 673
        Height = 309
        Align = alClient
        BevelOuter = bvNone
        Padding.Right = 2
        Padding.Bottom = 1
        TabOrder = 0
        object ColumnsDBGrid: TBCDBGrid
          Left = 0
          Top = 0
          Width = 671
          Height = 308
          Align = alClient
          AllowedSelections = [gstRecordBookmarks, gstAll]
          DataSource = ColumnsDataSource
          DrawGraphicData = True
          DrawMemoText = True
          DynProps = <>
          FooterParams.Color = clWindow
          IndicatorOptions = []
          Options = [dgEditing, dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 0
          OnGetCellParams = ColumnsDBGridGetCellParams
          Columns = <
            item
              DynProps = <>
              EditButtons = <>
              FieldName = 'COLUMN_NAME'
              Footers = <>
              Title.Caption = 'Column Name'
              Width = 150
            end
            item
              DynProps = <>
              EditButtons = <>
              FieldName = 'DATA_TYPE'
              Footers = <>
              PickList.Strings = (
                'BFILE'
                'BLOB'
                'CHAR'
                'CLOB'
                'DATE'
                'NCHAR'
                'NCLOB'
                'NUMBER'
                'NVARCHAR2'
                'RAW'
                'ROWID'
                'TIMESTAMP'
                'UROWID'
                'VARCHAR2'
                'XMLTYPE')
              Title.Caption = 'Data Type'
              Width = 100
            end
            item
              Alignment = taRightJustify
              DynProps = <>
              EditButtons = <>
              FieldName = 'COLUMN_SIZE'
              Footers = <>
              Title.Alignment = taRightJustify
              Title.Caption = 'Size'
              Width = 54
            end
            item
              Alignment = taRightJustify
              DynProps = <>
              EditButtons = <>
              FieldName = 'COLUMN_PRECISION'
              Footers = <>
              Title.Alignment = taRightJustify
              Title.Caption = 'Precision'
              Width = 54
            end
            item
              Alignment = taRightJustify
              DynProps = <>
              EditButtons = <>
              FieldName = 'SCALE'
              Footers = <>
              Title.Alignment = taRightJustify
              Title.Caption = 'Scale'
              Width = 54
            end
            item
              Checkboxes = True
              DynProps = <>
              EditButtons = <>
              FieldName = 'PRIMARY_KEY'
              Footers = <>
              KeyList.Strings = (
                'True'
                'False')
              Title.Alignment = taCenter
              Title.Caption = 'Primary Key'
              Width = 69
            end
            item
              Checkboxes = True
              DynProps = <>
              EditButtons = <>
              FieldName = 'NULLABLE'
              Footers = <>
              KeyList.Strings = (
                'True'
                'False')
              Title.Alignment = taCenter
              Title.Caption = 'Not Null'
              Width = 49
            end
            item
              DynProps = <>
              EditButtons = <>
              FieldName = 'COLUMN_DEFAULT'
              Footers = <>
              Title.Caption = 'Default Value'
              Width = 111
            end>
          object RowDetailData: TRowDetailPanelControlEh
          end
        end
      end
      object ColumnButtonPanel: TPanel
        Left = 0
        Top = 309
        Width = 673
        Height = 23
        Align = alBottom
        BevelOuter = bvNone
        Padding.Top = 1
        TabOrder = 1
        object ColumnsToolBar: TBCToolBar
          Left = 0
          Top = 1
          Width = 92
          Height = 22
          Align = alLeft
          Images = ImageList
          TabOrder = 0
          object MoveUpToolButton: TToolButton
            Left = 0
            Top = 0
            Action = MoveUpAction
          end
          object MoveDownToolButton: TToolButton
            Left = 23
            Top = 0
            Action = MoveDownAction
          end
          object AddColumnToolButton: TToolButton
            Left = 46
            Top = 0
            Action = AddColumnAction
          end
          object DeleteColumnToolButton: TToolButton
            Left = 69
            Top = 0
            Action = DeleteColumnAction
          end
        end
      end
    end
    object ColumnCommentsTabSheet: TTabSheet [1]
      Caption = 'Column Comments'
      ImageIndex = 2
      object ColumnCommentsPanel: TPanel
        Left = 0
        Top = 0
        Width = 673
        Height = 332
        Align = alClient
        BevelOuter = bvNone
        Padding.Right = 2
        Padding.Bottom = 1
        TabOrder = 0
        object ColumnCommentsDBGrid: TBCDBGrid
          Left = 0
          Top = 0
          Width = 671
          Height = 331
          Align = alClient
          DataSource = ColumnsDataSource
          DrawGraphicData = True
          DrawMemoText = True
          DynProps = <>
          FooterParams.Color = clWindow
          IndicatorOptions = []
          Options = [dgEditing, dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 0
          OnGetCellParams = ColumnCommentsDBGridGetCellParams
          Columns = <
            item
              Color = clBtnFace
              DynProps = <>
              EditButtons = <>
              FieldName = 'COLUMN_NAME'
              Footers = <>
              ReadOnly = True
              Title.Caption = 'Column Name'
              Width = 150
            end
            item
              DynProps = <>
              EditButtons = <>
              FieldName = 'COLUMN_COMMENT'
              Footers = <>
              Title.Caption = 'Column Comment'
              Width = 498
            end>
          object RowDetailData: TRowDetailPanelControlEh
          end
        end
      end
    end
    inherited SourceTabSheet: TTabSheet
      ExplicitWidth = 673
      ExplicitHeight = 332
      inherited SourcePanel: TPanel
        Width = 673
        Height = 332
        ExplicitWidth = 673
        ExplicitHeight = 332
        inherited SourceSynEdit: TSynEdit
          Width = 671
          Height = 307
          ExplicitWidth = 671
          ExplicitHeight = 307
        end
        inherited SourceTopPanel: TPanel
          Width = 671
          ExplicitWidth = 671
        end
      end
    end
  end
  inherited OKButton: TButton
    Left = 524
    Top = 427
    ExplicitLeft = 524
    ExplicitTop = 427
  end
  inherited CancelButton: TButton
    Left = 605
    Top = 427
    ExplicitLeft = 605
    ExplicitTop = 427
  end
  object TableNameEdit: TBCEdit [5]
    Left = 76
    Top = 9
    Width = 302
    Height = 21
    Hint = 'Table name'
    Anchors = [akLeft, akTop, akRight]
    CharCase = ecUpperCase
    TabOrder = 3
    OnChange = PageControlChange
    EnterToTab = False
    OnlyNumbers = False
    NumbersWithDots = False
    NumbersWithSpots = False
    ErrorColor = 14803198
    NumbersAllowNegative = False
  end
  object CommentEdit: TBCEdit [6]
    Left = 76
    Top = 33
    Width = 604
    Height = 21
    Hint = 'Comment'
    Anchors = [akLeft, akTop, akRight]
    TabOrder = 4
    OnChange = PageControlChange
    EnterToTab = False
    OnlyNumbers = False
    NumbersWithDots = False
    NumbersWithSpots = False
    ErrorColor = 14803198
    NumbersAllowNegative = False
  end
  inherited StatusBar1: TStatusBar
    Top = 458
    Width = 688
    ExplicitTop = 458
    ExplicitWidth = 688
  end
  inherited ActionList: TActionList
    Left = 283
    object MoveUpAction: TAction
      Enabled = False
      Hint = 'Move up'
      ImageIndex = 3
      OnExecute = MoveUpActionExecute
    end
    object MoveDownAction: TAction
      Enabled = False
      Hint = 'Move down'
      ImageIndex = 4
      OnExecute = MoveDownActionExecute
    end
    object AddColumnAction: TAction
      Hint = 'Add column'
      ImageIndex = 5
      OnExecute = AddColumnActionExecute
    end
    object DeleteColumnAction: TAction
      Enabled = False
      Hint = 'Delete column'
      ImageIndex = 6
      OnExecute = DeleteColumnActionExecute
    end
  end
  inherited SynSQLSyn: TSynSQLSyn
    Left = 276
    Top = 60
  end
  inherited ImageList: TBCImageList
    Bitmap = {
      
      000000000000}
  end
  object ColumnsDataSource: TOraDataSource
    DataSet = ColumnsQuery
    Left = 294
    Top = 152
  end
  object ColumnsQuery: TOraQuery
    LocalUpdate = True
    SQL.Strings = (
      
        'SELECT LPAD('#39' '#39', 30) column_name, LPAD('#39' '#39', 106) data_type, 0 co' +
        'lumn_size, 0 column_precision,'
      
        '       0 scale, LPAD('#39' '#39', 5) primary_key, LPAD('#39' '#39', 5) nullable,' +
        ' LPAD('#39' '#39', 4096) column_default,'
      '       LPAD('#39' '#39', 4096) column_comment'
      '  FROM dual'
      ' WHERE 1=0')
    Options.StrictUpdate = False
    AfterScroll = ColumnsQueryAfterScroll
    OnNewRecord = ColumnsQueryNewRecord
    Left = 300
    Top = 220
  end
end
